...
**Current release: [v1.7.15](https://github.com/MarkWSavage/PANDA/releases/tag/v1.7.15) — CREME-MC Shoulder: PENELOPE Explanation Retracted.** Follow-up to v1.7.14: the previously-proposed explanation for PANDA's ~75x direct-ionization cross-section excess vs. CREME-MC in a narrow 0.1-0.5 fC window ("CREME-MC's engine hands electron transport to PENELOPE, vs. PANDA's native Geant4 EM physics") was checked and found wrong. PANDA's own EM constructor (`G4EmStandardPhysics_option4`, `PANDA.cc`) already uses Penelope ionisation models for electrons down to a 100 eV energy limit, fully covering the 2-11 keV delta-ray range identified as the shoulder's dominant contributor (83% of shoulder events carry an explicit delta-ray hit, averaging 91% of that event's deposited energy -- unchanged from v1.7.14, still a confirmed, correctly-functioning secondary-transport effect, not primary-track straggling or a production-cut artifact). The magnitude gap itself now has no confirmed explanation. Documentation only -- no functional or behavioral code changes. See `Documentation/PANDA_VALIDATION_SUMMARY.md` Section 1.5 for the full writeup.
...
- **CREME-MC thin-film direct-ionization shoulder, magnitude unexplained** (found 2026-07-23/24, PENELOPE explanation retracted 2026-07-24): a 200 MeV proton through a 0.5um sensitive Si / 0.5um SiO2 dead layer stack shows PANDA's cross section running up to ~75x higher than CREME-MC's in a narrow 0.1-0.5 fC window, converging again by ~1-2 fC. Confirmed via a per-event `events.csv` breakdown that this window is genuinely delta-ray-dominated inside PANDA (83% of events carry an explicit knock-on-electron hit, averaging 91% of that event's deposited energy) -- not primary-track straggling, and not a production-cut artifact (the region-scoped cut here is 50nm, a 10x margin under the 500nm sensitive layer). The original hypothesis -- that CREME-MC's engine (MRED) hands electron transport to PENELOPE while PANDA uses native Geant4 EM physics -- is wrong: PANDA's own EM constructor (`G4EmStandardPhysics_option4`) already uses Penelope ionisation models for electrons down to 100 eV, covering this exact energy range. The magnitude gap has no confirmed explanation. No evidence of a PANDA defect; see `Documentation/PANDA_VALIDATION_SUMMARY.md` Section 1.5.
