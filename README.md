...
- **Heavy-ion test energy should stay realistic (>~1 GeV for Au-mass ions)** (found 2026-07-22): 200 MeV Au197 (specific energy ~1 MeV/nucleon) produces an anomalously narrow, near-delta-function differential charge spectrum (relative std 2.96%) rather than genuine Vavilov/Landau straggling -- a low-reduced-velocity limitation of Geant4's own energy-loss fluctuation sampling for very slow heavy ions, separate from the mean-LET model swap in v1.7.10/v1.7.11. Confirmed by rerunning the identical geometry at 3000 MeV (~15 MeV/nucleon, within real cyclotron heavy-ion SEE test-beam ranges): the spectrum becomes a properly-spread peak (relative std 8.30%), well clear of the max-possible-charge edge. Not a PANDA-specific bug -- just a reminder to pick physically-achievable test energies. See `Documentation/PANDA_VALIDATION_SUMMARY.md` Section 2.
