...
**Current release: [v1.7.20](https://github.com/MarkWSavage/PANDA/releases/tag/v1.7.20) — Linear-Threshold Cross-Section Plots + Geometry Annotation.** The log-x cross-section plots mirror CREME-MC convention, but a specific circuit's actual upset threshold isn't tied to that log spacing -- it can fall anywhere. Each cross-section plot now has a linear-x companion (`cumulative_cross_section_{deposited,collected}_linear.png`) alongside the existing log version, so a given Qcrit can be read off directly. Every plot (differential spectrum, cumulative probability, and both cross-section variants) also now carries a text box stamping the run's particle, energy, and sensitive/dead-layer geometry, so a saved figure is self-describing without cross-referencing `run.mac`. `Results/Current` has also been refreshed for a new representative device geometry (.25/.25/.5 um sensitive, 5 um dead layer, 200 MeV protons).
