...
**Current release: [v1.7.12](https://github.com/MarkWSavage/PANDA/releases/tag/v1.7.12) — ICRU73 Fe-Scaling Documentation Fix and 16 MeV/u Cocktail SRIM Revalidation.** Ran a fresh SRIM cross-check using the standard 16 MeV/u heavy-ion cocktail energies (C12@200, F19@305, Cl35@560, Ni58@928, I127@2032, Au197@3152 MeV) in a new thin geometry (10 um sensitiveXY, 0.5 um dead SiO2, 1 um sensitive Si), comparing against SRIM's own depth-resolved ionization profile integrated over just the sensitive layer rather than a single entrance-energy dE/dx lookup. F19/Cl35/Ni58/I127 agreed within ~7%; C12 (-8.4%) and Au197 (+10.2%) stood out as the two Z-extremes. Traced both, via Geant4 11.4.1 source: `G4IonDEDXScalingICRU73`'s default Z>=19 threshold silently substitutes iron's (Z=26) ICRU73 table, scaled by effective-charge^2, for any heavier ion -- meaning Au197 never actually reads its own `z79_14.dat` file in a single-element target. C12 (Z=6) is below that threshold and uses genuine native ICRU73 data, so its gap is ordinary cross-database variance; Au197's is a real, structural Fe-scaling limitation that grows with distance from Fe in Z. Corrected a `PANDA.cc` comment that previously implied the earlier 200 MeV Au197 ICRU73 check validated against real per-ion data, when it was always checking the Fe-scaled value. Documentation/validation only -- no functional or behavioral code changes. See `Documentation/PANDA_MASTER_DESIGN` (Section 5, Version History) for what's frozen and what's in scope.
