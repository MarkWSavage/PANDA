...
**Current release: [v1.7.16](https://github.com/MarkWSavage/PANDA/releases/tag/v1.7.16) — CREME-MC Shoulder: Production-Cut-Granularity Explanation Also Ruled Out.** Follow-up to v1.7.15: tested whether MRED using a coarser production cut than PANDA's 50nm (for this thin-film geometry) could explain PANDA's ~75x direct-ionization cross-section excess vs. CREME-MC in the 0.1-0.5 fC window. Rebuilt PANDA in an isolated scratch copy with the cut hardcoded to Geant4's stock 0.7mm default and reran the identical geometry: this eliminates explicit delta-ray secondaries entirely (0% of shoulder events carry an `Electron_keV` hit, vs. 82.5% at 50nm), confirming the override worked -- but the cross-section tail gets *heavier*, not lighter (4-250x higher above 0.4 fC), the wrong direction to explain CREME-MC's lower values. Mechanism: an explicit delta-ray secondary can physically escape this ~1um-thick stack, carrying energy out of the scored volume; forcing continuous local deposit instead guarantees 100% in-volume containment, inflating the tail. Both major transport-family candidates (electron-ionisation model choice, v1.7.15; production-cut granularity, this release) are now ruled out; the magnitude gap has no confirmed explanation. Documentation only -- no functional or behavioral code changes. See `Documentation/PANDA_VALIDATION_SUMMARY.md` Section 1.5 for the full writeup.
