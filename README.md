...
**Current release: [v1.7.17](https://github.com/MarkWSavage/PANDA/releases/tag/v1.7.17) — Native ICRU73 Stopping Data for Far-From-Fe Heavy Ions.** `G4IonDEDXScalingICRU73` (used by `G4IonParametrisedLossModel`) redirects every ion with Z>=19 to Fe's (Z=26) scaled stopping-power table before ever reading that ion's own file -- confirmed via Geant4 11.4.1 source that this is hardcoded, not a data-availability gap: `G4EMLOW8.8` ships genuine native ICRU73 files for every Z from 3 to 80, including Au197's own `z79_14.dat`. Tried disabling the redirect for the whole Z>=19 range first and validated against SRIM: Au197 improved (+17.8% -> +8.4%), but Ni58 (Z=28, only 2 above Fe) got *worse* (+14.7% -> +17.8%) and I127 (Z=53) flipped sign at similar magnitude -- Fe's scaling approximation is already good for an ion this close to it, so a blanket switch isn't a strict improvement. New `NativeIonStoppingPhysics` scopes this to Z>66 (the midpoint between I127, which stays Fe-scaled, and Au197, which doesn't) -- Ni58/I127 numerically unchanged, only Au197 (and any future primary similarly far from Fe) gets native data. See `Documentation/PANDA_VALIDATION_SUMMARY.md` Section 1.6 for the full writeup.
...
- **CREME-MC thin-film direct-ionization shoulder, magnitude unexplained** (found 2026-07-23/24; PENELOPE explanation retracted, production-cut-granularity explanation ruled out, 2026-07-24): a 200 MeV proton through a 0.5um sensitive Si / 0.5um SiO2 dead layer stack shows PANDA's cross section running up to ~75x higher than CREME-MC's in a narrow 0.1-0.5 fC window, converging again by ~1-2 fC. Confirmed via a per-event `events.csv` breakdown that this window is genuinely delta-ray-dominated inside PANDA (83% of events carry an explicit knock-on-electron hit, averaging 91% of that event's deposited energy) -- not primary-track straggling. Two proposed explanations for the magnitude gap were both checked directly and ruled out: (1) "MRED uses PENELOPE, PANDA uses native Geant4 EM physics" -- wrong, PANDA's own EM constructor (`G4EmStandardPhysics_option4`) already uses Penelope ionisation for electrons down to 100 eV, covering this exact energy range; (2) "MRED uses a coarser production cut" -- tested by rebuilding PANDA with a hardcoded 0.7mm cut on the identical geometry, which eliminates explicit delta-ray secondaries entirely but makes the cross-section tail *heavier* (4-250x above 0.4 fC), the wrong direction to explain CREME-MC's lower values. The magnitude gap has no confirmed explanation. No evidence of a PANDA defect; see `Documentation/PANDA_VALIDATION_SUMMARY.md` Section 1.5.

- **Ni58/I127 ICRU73-vs-SRIM gaps are genuine model variation, not a bug** (confirmed 2026-07-24): fixing Au197's Fe-scaling issue (this release, see above) required directly testing whether the same fix helps Ni58 (Z=28) and I127 (Z=53) too -- it doesn't. Both stayed on Fe-scaling deliberately after native data was confirmed to make Ni58 worse (+14.7% -> +17.8% vs SRIM) and flip I127's sign at similar magnitude. Their residual SRIM gaps (Ni58 ~14.7%, I127 ~6.4%, see `Documentation/PANDA_VALIDATION_SUMMARY.md` Section 1.4) are ordinary Geant4-EM-vs-SRIM stopping-power model variation for ions reasonably close to Fe in Z, not something a scaling-table fix can close further.
